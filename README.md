...
答案选项乱序默认开启，每次开始刷题都会重新打乱单选/多选题的选项位置，并同步重映射正确答案。例如原题 A 为 1、B 为 2、C 为 3，启用后本次会话可能显示为 A 为 2、B 为 1、C 为 3，判题仍按重排后的正确位置计算。也可以在“刷题设置”的“操作”区域关闭；模拟考试中该设置会隐藏并强制开启。

## 题库加载机制

页面打开时只读取 `data/question-banks.json` 题库清单，不会立即读取完整题目 JSON。用户点击“开始”或“重载”时，才会加载当前选中的题库；带 URL 参数的直达链接会自动加载，以保留分享链接直接应用筛选或自动开始的能力。

单个题库超过 1000 题时，筛选结果会缓存，答题卡也只渲染首尾和当前题附近的题号，避免一次性创建过多 DOM 节点。纯前端静态 JSON 仍需要在开始时完整下载当前题库；如果题库继续增长到数万题，建议按分类或章节拆成多个题库文件，放到题库清单中按需选择。
